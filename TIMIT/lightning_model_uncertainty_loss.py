...
class LightningModel(pl.LightningModule):
    def __init__(self, HPARAMS):
        ...
        self.mae_criterion = MeanAbsoluteError()
        self.rmse_criterion = RMSELoss()
        self.gender_acc = BinaryAccuracy(threshold=0.5)
        ...
        print(
            f"Model Details: #Params = {self.count_total_parameters()}\t"
            f"#Trainable Params = {self.count_trainable_parameters()}"
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self.uncertainty_loss(
            torch.cat((y_hat_h, y_hat_a, y_hat_g)),
            torch.cat((y_h, y_a, y_g))
        )
        ...
        # y_hat_g is probability; y_g is 0/1 float
        gender_acc = self.gender_acc(y_hat_g, y_g.int())

        self.log('train/loss', loss, on_step=False, on_epoch=True, prog_bar=True)
        self.log('train/h', height_mae, on_step=False, on_epoch=True, prog_bar=True)
        self.log('train/a', age_mae, on_step=False, on_epoch=True, prog_bar=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log('val/h', height_mae, on_step=False, on_epoch=True, prog_bar=True)
        self.log('val/a', age_mae, on_step=False, on_epoch=True, prog_bar=True)
    ...
    def test_step(self, batch, batch_idx):
        ...
        male_height_mae = self.mae_criterion(
            y_hat_h[male_idx] * self.h_std + self.h_mean, y_h[male_idx] * self.h_std + self.h_mean
        )
        male_age_mae = self.mae_criterion(
            y_hat_a[male_idx] * self.a_std + self.a_mean, y_a[male_idx] * self.a_std + self.a_mean
        )

        female_height_mae = self.mae_criterion(
            y_hat_h[female_idx] * self.h_std + self.h_mean, y_h[female_idx] * self.h_std + self.h_mean
        )
        female_age_mae = self.mae_criterion(
            y_hat_a[female_idx] * self.a_std + self.a_mean, y_a[female_idx] * self.a_std + self.a_mean
        )

        male_height_rmse = self.rmse_criterion(
            y_hat_h[male_idx] * self.h_std + self.h_mean, y_h[male_idx] * self.h_std + self.h_mean
        )
        male_age_rmse = self.rmse_criterion(
            y_hat_a[male_idx] * self.a_std + self.a_mean, y_a[male_idx] * self.a_std + self.a_mean
        )

        female_height_rmse = self.rmse_criterion(
            y_hat_h[female_idx] * self.h_std + self.h_mean, y_h[female_idx] * self.h_std + self.h_mean
        )
        female_age_rmse = self.rmse_criterion(
            y_hat_a[female_idx] * self.a_std + self.a_mean, y_a[female_idx] * self.a_std + self.a_mean
        )

        self.log('test/g', gender_acc, on_step=False, on_epoch=True, prog_bar=True)
        ...
        return {
            ...
            'female_height_rmse': female_height_rmse,
